  ...
  return (
    <div className="xl:w-[72rem] m-auto mt-[36px] xl:mt-[3.75rem] w-auto">
      <div className="px-[20px]">
        <h3 className="text-center xl:text-[2.125rem] text-white text-base font-semibold xl:mb-[20px]">{`Reader's Section`}</h3>
        <p className="text-center text-sm xl:text-xl text-white xl:leading-[25px] xl:mb-[1.87rem] mb-[1rem] font-normal">
      ...
      <div className="px-[20px] md:px-[0px] md:pb-4 no-scrollbar flex xl:gap-[30px] xl:overflow-hidden md:overflow-y-hidden items-start overflow-x-scroll gap-[14px] justify-between">
        {data.slice(0, 4).map((d: any, index: any) => (
          <GodCard loginToken={loginToken} key={index} data={d} />
        ))}
      </div>
    </div>
  );
